...
<header>
    <h1>🏆 Vainqueurs de la coupe du monde de Football v5</h1>
</header>
...
<div class="container">
    ...
    $teamsTrier = array_unique(array_merge(...array_values($winners)));
    foreach ($teamsTrier as $teamName) {
        $selected = (isset($_GET['teamName']) && $_GET['teamName'] == $teamName) ? 'selected' : '';
    ...
    elseif (!empty($_GET['teamName'])){
        $teamName = $_GET['teamName'];
        foreach ($winners as $year => $teams) {
            if (in_array($teamName, $teams)) {
                echo "<div class='winner-card'>";
                echo "<h2>$year</h2>";
                echo displayTeam($teamName, $countryCodes);
                echo "</div>";
            }
        }
    } else {
